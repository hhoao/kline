...
public enum ColumnType {
    ...
    public static List<ColumnType> TIME_TYPE =
            Arrays.asList(DATE, DATETIME, DATETIME2, SIMPLEDATETIME, TIME, TIMESTAMP);
    ...
    public static List<ColumnType> COMPLEX_TYPE = Arrays.asList(MAP, ARRAY, STRUCT, OBJECT);

    public static boolean isComplexType(ColumnType type) {
        return COMPLEX_TYPE.contains(type);
    }
    ...
    public static ColumnType fromString(String type) {
        ...
        if (type.startsWith("ARRAY") || type.startsWith("MAP") || type.startsWith("STRUCT")) {
            int position = type.indexOf(ConstantValue.LESS_THAN_SIGN);
            if (position > 0) {
