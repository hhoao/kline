...
package com.hhoa.kline.plugins.jdbc.types.logical;
...
public final class TimestampType extends LogicalType {
    ...
    private static final Set<String> INPUT_OUTPUT_CONVERSION =
            conversionSet(
                    java.sql.Timestamp.class.getName(), java.time.LocalDateTime.class.getName());
    ...
    public TimestampType(boolean isNullable, TimestampKind kind, int precision) {
        ...
        if (precision >= 0 && precision <= 9) {
            if (kind == TimestampKind.PROCTIME) {
                throw new ValidationException("PgTimestampType can not be used as PROCTIME type.");
            } else {
                this.kind = kind;
