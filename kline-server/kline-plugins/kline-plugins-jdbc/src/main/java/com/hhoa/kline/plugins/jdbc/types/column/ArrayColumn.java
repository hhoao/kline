...
import static com.hhoa.kline.plugins.jdbc.types.logical.utils.LogicalTypeChecks.getPrecision;
...
public class ArrayColumn extends AbstractBaseColumn {
    ...
    public ArrayColumn(Object data, int size, LogicalType elementType) {
        this(data, size, elementType.getDefaultConversion().isPrimitive());
    }

    public ArrayColumn(Object data, LogicalType elementType) {
        this(data, elementType, elementType.getDefaultConversion().isPrimitive());
    }
    ...
    private void checkAndSet(LogicalType elementType) {
        ...
        if (o instanceof int[]) {
            this.byteSize = ((int[]) o).length * 4;
        } else if (o instanceof long[]) {
            this.byteSize = ((long[]) o).length * 8;
        } else if (o instanceof double[]) {
            this.byteSize = ((double[]) o).length * 8;
        } else if (o instanceof float[]) {
            this.byteSize = ((float[]) o).length * 8;
        } else if (o instanceof short[]) {
            this.byteSize = ((short[]) o).length * 2;
        } else if (o instanceof byte[]) {
            this.byteSize = ((byte[]) o).length;
        } else if (o instanceof boolean[]) {
            this.byteSize = ((boolean[]) o).length;
        } else if (o instanceof Integer[]) {
            this.byteSize = ((Integer[]) o).length * 4;
        } else if (o instanceof Long[]) {
            this.byteSize = ((Long[]) o).length * 8;
        } else if (o instanceof Double[]) {
            this.byteSize = ((Double[]) o).length * 8;
            ...
            this.byteSize = ((String[]) o).length * 8;
        } else if (o instanceof BigDecimal[]) {
            this.byteSize = ((BigDecimal[]) o).length * 12;
        } else if (o instanceof Date[]) {
            this.byteSize = ((Date[]) o).length * 8;
        } else if (o instanceof Time[]) {
            this.byteSize = ((Time[]) o).length * 8;
        } else if (o instanceof Timestamp[]) {
            int precision = getPrecision(elementType);
            if (precision > 3) {
                this.byteSize = ((Timestamp[]) o).length * 12;
            } else {
                this.byteSize = ((Timestamp[]) o).length * 8;
            }
        } else {
    ...
    public Boolean asBooleanInternal() {
        throw new CastException(this.type(), "BOOLEAN", this.asStringInternal());
    }

    @Override
    public byte[] asBytesInternal() {
        throw new CastException(this.type(), "BYTES", this.asStringInternal());
    }
    ...
    public BigDecimal asBigDecimalInternal() {
        throw new CastException(this.type(), "BigDecimal", this.asStringInternal());
    }

    @Override
    public Timestamp asTimestampInternal() {
        throw new CastException(this.type(), "java.sql.Timestamp", this.asStringInternal());
    }

    @Override
    public Time asTimeInternal() {
        throw new CastException(this.type(), "java.sql.Time", this.asStringInternal());
    }

    @Override
    public Date asSqlDateInternal() {
        throw new CastException(this.type(), "java.sql.Date", this.asStringInternal());
    }
