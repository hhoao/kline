...
public class ZonedTimestampColumn extends AbstractBaseColumn {
    ...
    private static final int DEFAULT_MILLISECOND_OFFSET = TimeZone.getDefault().getRawOffset();
    ...
    public ZonedTimestampColumn(Timestamp data) {
        ...
        this.millisecondOffset = DEFAULT_MILLISECOND_OFFSET;
        this.precision = 6;
    }
    ...
    public Timestamp asTimestampInternal() {
        Timestamp timestamp =
                new Timestamp((long) data - DEFAULT_MILLISECOND_OFFSET + millisecondOffset);
        if (precision > 3) {
            timestamp.setNanos(nanos);
        }
        return timestamp;
    ...
    public Timestamp asTimestampWithTimeZone(TimeZone timeZone) {
        ...
        Timestamp timestamp =
                new Timestamp((long) data - DEFAULT_MILLISECOND_OFFSET + timeZone.getRawOffset());
        if (precision > 3) {
            timestamp.setNanos(nanos);
        }
        return timestamp;
