...
/**
 * GLM-specific component override templates. Provides optimized prompt text for GLM model family.
 *
 * @author hhoa
 */
public final class GlmComponentOverrides {

    private GlmComponentOverrides() {}

    private static String glmToolUseTemplate(SystemPromptContext context) {
        boolean hasMcpServers =
                context.getMcpHub() != null && context.getMcpHub().getServers() != null;
        ...
        return """
                ...
                **execute_command** — Run terminal commands in {{CWD}} or other directories.\s\s
                ...
                **attempt_completion** — Final result (no questions). Use this tool only when all goals have been completed.
                Params: result, command (optional demonstration of completed work).\s\s
                *Example:*
                <attempt_completion>
                ...
                **plan_mode_respond** — PLAN-only reply.
                Params: response, needs_more_exploration (optional).\s\s
    ...
    private static final String GLM_OBJECTIVE_TEMPLATE =
            """
            OBJECTIVE
    ...
    private static final String GLM_TASK_PROGRESS_TEMPLATE =
            """
    ...
    private static String glmRulesTemplate(SystemPromptContext context) {
        ...
        String outputRule =
                yoloMode
                        ? ""
                        : " If you must see output, use ask_followup_question to request a pasted log.";
        ...
        return """
                ...
                - %s%s
                - If command output doesn't appear, assume success and continue.%s
                ...
                """
                .formatted(askRule, vagueRule, outputRule);
    }

    public static Map<SystemPromptSection, ConfigOverride> getOverrides() {
        return Map.of(
                SystemPromptSection.OBJECTIVE,
                ConfigOverride.create().template(GLM_OBJECTIVE_TEMPLATE),
                SystemPromptSection.TOOL_USE,
                ConfigOverride.create().templateFunction(GlmComponentOverrides::glmToolUseTemplate),
                SystemPromptSection.RULES,
                ConfigOverride.create().templateFunction(GlmComponentOverrides::glmRulesTemplate),
                SystemPromptSection.TASK_PROGRESS,
                ConfigOverride.create().template(GLM_TASK_PROGRESS_TEMPLATE),
                SystemPromptSection.MCP,
                ConfigOverride.create().template(GLM_MCP_TEMPLATE));
    }
}
