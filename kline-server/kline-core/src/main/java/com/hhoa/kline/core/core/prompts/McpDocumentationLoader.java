...
/**
 * MCP 文档加载服务 生成 MCP 服务器文档
 *
 * @author hhoa
 */
@Slf4j
public class McpDocumentationLoader {

    public String loadMcpDocumentation(
            String mcpServersPath, String mcpSettingsFilePath, List<McpServer> connectedServers) {
        String serverNames = "(None running currently)";
        if (connectedServers != null && !connectedServers.isEmpty()) {
            String joined =
                    connectedServers.stream()
        ...
        Unless the user specifies otherwise, new MCP servers should be created in: %s
        ...
        5. Install the MCP Server by adding the MCP server configuration to the settings file located at '%s'. The settings file may have other MCP servers already configured, so you would read it first and then add your new server to the existing `mcpServers` object.
        ...
        The user may ask to add tools or resources that may make sense to add to an existing MCP server (listed under 'Connected MCP Servers' below: %s), e.g. if it would use the same API. This would be possible if you can locate the MCP server repository on the user's system by looking at the server arguments for a filepath. You might then use list_files and read_file to explore the files in the repository, and use replace_in_file to make changes to the files.
        ...
        """
                .formatted(mcpServersPath, mcpServersPath, mcpSettingsFilePath, serverNames);
    }
    ...
    public static class McpServer {
        ...
        public McpServer(String name, String status) {
            this.name = name;
            this.status = status;
        }
