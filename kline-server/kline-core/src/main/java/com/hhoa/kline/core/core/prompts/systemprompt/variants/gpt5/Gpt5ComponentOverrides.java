...
/**
 * GPT-5 specific component override templates.
 *
 * @author hhoa
 */
public final class Gpt5ComponentOverrides {

    private Gpt5ComponentOverrides() {}
    ...
    private static String rulesTemplate(SystemPromptContext context) {
        ...
        return MessageFormat.format("""
            ...
            - {0}. For example, if the user mentions a file that may be in an outside directory like the Desktop, you should use the list_files tool to list the files in the Desktop and check if the file they are talking about is there, rather than asking the user to provide the file path themselves.{1}
            - When executing commands, if you don''t see the expected output, assume the terminal executed the command successfully and proceed with the task. The user''s terminal may be unable to stream the output back properly.{2}
            ...
            - You may use multiple tools in a single response when the operations are independent (e.g., reading several files, creating independent files). For dependent operations where one result informs the next, use tools sequentially and wait for the user''s response.'{{'BROWSER_WAIT_RULES'}}'{3}""",
            askRule, vagueRule, outputRule, mcpRule);
    }

    public static Map<SystemPromptSection, ConfigOverride> getOverrides() {
        return Map.of(
                SystemPromptSection.RULES,
                ConfigOverride.create().templateFunction(Gpt5ComponentOverrides::rulesTemplate));
    }
}
