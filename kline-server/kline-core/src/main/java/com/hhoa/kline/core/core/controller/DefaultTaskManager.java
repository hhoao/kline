...
public class DefaultTaskManager implements TaskManager {
    ...
    private final Map<String, TaskV2> tasks = new HashMap<>();
    private final FocusChainManagerFactory focusChainManagerFactory;
                        ...
                        new ThreadFactory() {
                            ...
                            public Thread newThread(Runnable r) {
                                Thread t =
                                        new Thread(
                                                r,
                                                "DefaultTaskManager-TaskExecutor-"
    ...
    public String initTask(
        ...
        try {
            ...
            boolean taskLockAcquired = lockResult.acquired();
            if (lockResult.acquired()) {
                log.debug("[TaskV2 {}] Task lock acquired", taskId);
            } else {
                log.debug("[TaskV2 {}] Task lock skipped", taskId);
            }
            ...
            if (taskParams.getHistoryItem() != null) {
                CompletableFuture.runAsync(
                        () -> taskContext.run(() -> task.handle(new ResumeTaskEvent(taskId))),
                        taskExecutor);
            ...
            log.error("[DefaultTaskManager] 初始化任务失败", e);
    ...
    public TaskV2 getTask(String taskId) {
        return tasks.get(taskId);
    }

    private TaskV2 getOrInitTask(String taskId) {
    ...
    public void exportTaskWithId(String id) {
        log.info("[DefaultTaskManager] 导出任务: {}", id);
    }
        ...
        if (taskExecutor != null) {
            ...
            try {
                if (!taskExecutor.awaitTermination(30, TimeUnit.SECONDS)) {
                    log.warn("[DefaultTaskManager] 等待任务完成超时，强制关闭线程池");
                    taskExecutor.shutdownNow();
                }
            } catch (InterruptedException e) {
                taskExecutor.shutdownNow();
                Thread.currentThread().interrupt();
                log.error("[DefaultTaskManager] 关闭线程池时被中断", e);
            }
        }

        log.info("[DefaultTaskManager] DefaultTaskManager disposed");
    }
    ...
    @Override
    public void handleWebviewAskResponse(
            ...
            List<String> images,
            List<String> files) {
        TaskV2 task = getOrInitTask(taskId);
        task.handle(
                new UserRespondedEvent(
                        taskId,
                        pendingId,
                        new AskResult(responseType, text, images, pendingId, files)));
    }

    @Override
    public void restoreCheckpoint(String taskId, Long number, String restoreType, Long offset) {
        TaskV2 task = getOrInitTask(taskId);
        ...
        if (!task.getTaskState().isInitialized()) {
            log.warn("任务 {} 在 3000ms 内未完成初始化，仍尝试恢复检查点", taskId);
        }
        ...
        task.getCheckpointManager()
                .restoreCheckpoint(number, restoreType, offset.intValue())
                .join();
    }

    @Override
    public IMcpHub getMcpHub() {
        return mcpHub;
    }
