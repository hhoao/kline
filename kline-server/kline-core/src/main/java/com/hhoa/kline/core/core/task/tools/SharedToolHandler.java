...
public class SharedToolHandler implements FullyManagedTool {
    private final ClineDefaultTool name;
    private final FullyManagedTool baseHandler;

    public SharedToolHandler(ClineDefaultTool name, FullyManagedTool baseHandler) {
        this.name = name;
    ...
    public List<UserContentBlock> execute(TaskConfig config, ToolUse block) {
        return baseHandler.execute(config, block);
    }
}
