...
/**
 * Trinity variant configuration - Prompt optimized for Trinity models
 *
 * @author hhoa
 */
@Slf4j
public class TrinityVariantConfig {
    ...
    public static PromptVariant createVariant() {
        ...
        placeholders.put("MODEL_FAMILY", ModelFamily.TRINITY.name());

        return PromptVariant.buildAndValidate(
                PromptVariant.builder()
                        .family(ModelFamily.TRINITY)
                        .description(
                                "Prompt optimized for Trinity models with tool-use optimizations "
                                        + "(explicit ask_followup_question question parameter, anti-looping reminder).")
                        .version(1)
                        .tags(Arrays.asList("trinity", "stable"))
                        .labels(labels)
                        .matcher(context -> isTrinityModelFamily(getModelId(context)))
                        .componentOrder(COMPONENT_ORDER)
                        .tools(TOOLS)
                        .placeholders(placeholders)
                        .componentOverrides(TrinityComponentOverrides.getOverrides())
    ...
    static {
        ...
        ConfigTemplate.validateVariantConfig(built, "trinity");
        CONFIG = built;
    }
