...
/**
 * Devstral variant configuration - Baseline prompt for Devstral family models
 *
 * @author hhoa
 */
@Slf4j
public class DevstralVariantConfig {
    ...
    public static PromptVariant createVariant() {
        ...
        placeholders.put("MODEL_FAMILY", ModelFamily.DEVSTRAL.name());

        return PromptVariant.buildAndValidate(
                PromptVariant.builder()
                        .family(ModelFamily.DEVSTRAL)
                        .description("Baseline prompt for Devstral family models")
                        .version(1)
                        .tags(Arrays.asList("devstral", "stable"))
                        .labels(labels)
                        .matcher(context -> isDevstralModelFamily(getModelId(context)))
                        .componentOrder(COMPONENT_ORDER)
                        .tools(TOOLS)
                        .placeholders(placeholders)
                        .componentOverrides(DevstralComponentOverrides.getOverrides())
    ...
    static {
        ...
        ConfigTemplate.validateVariantConfig(built, "devstral");
        CONFIG = built;
    }
