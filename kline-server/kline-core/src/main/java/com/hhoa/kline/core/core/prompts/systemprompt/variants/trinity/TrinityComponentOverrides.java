package com.hhoa.kline.core.core.prompts.systemprompt.variants.trinity;
...
/**
 * Trinity-specific component override templates. Provides optimized prompt text for Trinity model
 * family with tool-use optimizations (explicit ask_followup_question question parameter,
 * anti-looping reminder, strict one-tool-per-message and XML-only rules).
 *
 * @author hhoa
 */
public final class TrinityComponentOverrides {

    private TrinityComponentOverrides() {}

    /**
     * Trinity-specific TOOL_USE override. Adds CRITICAL REQUIREMENTS and strict
     * one-tool-per-message, XML-only rules.
     */
    private static String trinityToolUseTemplate(SystemPromptContext context) {
        return """
                TOOL USE
    ...
    /**
     * Trinity-specific RULES override. Adds optimizations for Trinity models:
     *
     * <ul>
     *   <li>ask_followup_question: make the required "question" parameter explicit
     *   <li>Anti-looping: subtle reminder to check results before repeating the same tool
     *   <li>Strict XML-only and one-tool-per-message rules with examples
     * </ul>
     */
    private static String trinityRulesTemplate(SystemPromptContext context) {
        ...
        String yoloAskRule =
                yoloMode
                        ...
                        : "You are only allowed to ask the user questions using the"
                                ...
                                + " additional details to complete a task, and be sure to use a"
                                + " clear and concise question that will help you move forward with"
                                + " the task. However if you can use the available tools to avoid"
                                + " having to ask the user questions, you should do so";

        String yoloOutputRule =
                yoloMode
                        ? ""
                        : " If you absolutely need to see the actual terminal output, use the"
                                + " ask_followup_question tool to request the user to copy and"
        ...
        return """
                ...
                .formatted(yoloAskRule, yoloOutputRule);
    }
    ...
    public static Map<SystemPromptSection, ConfigOverride> getOverrides() {
        return Map.of(
                ...
                ConfigOverride.create()
                        .templateFunction(TrinityComponentOverrides::trinityToolUseTemplate),
                SystemPromptSection.RULES,
                ConfigOverride.create()
                        .templateFunction(TrinityComponentOverrides::trinityRulesTemplate));
    }
}
