...
public class RulesComponent implements SystemPromptComponent {

    private final TemplateEngine templateEngine;

    @Override
    public String apply(PromptVariant variant, SystemPromptContext context) {
        String template = getTemplateText(context);

        if (variant.getComponentOverrides() != null
                && variant.getComponentOverrides().containsKey(SystemPromptSection.RULES)) {
            var override = variant.getComponentOverrides().get(SystemPromptSection.RULES);
    ...
    public SystemPromptSection getSystemPromptSection() {
        return SystemPromptSection.RULES;
    }

    private String getTemplateText(SystemPromptContext context) {
        String yoloModeText =
    ...
    private String getBrowserRules() {
        return "- The user may ask generic non-development tasks, such as \"what's the latest news\" or \"look up the weather in San Diego\", in which case you might use the browser_action tool to complete the task if it makes sense to do so, rather than trying to create a website or using curl to answer the question. However, if an available MCP server tool or resource can be used instead, you should prefer to use it over browser_action.\n";
    }

    private String getBrowserWaitRules() {
        return " Then if you want to test your work, you might use browser_action to launch the site, wait for the user's response confirming the site was launched along with a screenshot, then perhaps e.g., click a button to test functionality if needed, wait for the user's response confirming the button was clicked along with a screenshot of the new state, before finally closing the browser.";
    }
}
