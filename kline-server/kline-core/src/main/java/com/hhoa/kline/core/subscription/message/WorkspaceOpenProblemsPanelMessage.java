package com.hhoa.kline.core.subscription.message;

import com.hhoa.kline.core.core.shared.proto.host.OpenProblemsPanelRequest;
import com.hhoa.kline.core.subscription.MessageType;
import com.hhoa.kline.core.subscription.SubscriptionRequestMessage;
import lombok.AllArgsConstructor;
import lombok.Data;

@Data
@AllArgsConstructor
public class WorkspaceOpenProblemsPanelMessage implements SubscriptionRequestMessage {
    private OpenProblemsPanelRequest request;

    @Override
    public MessageType getType() {
        return MessageType.WORKSPACE_OPEN_PROBLEMS_PANEL;
    }
}
