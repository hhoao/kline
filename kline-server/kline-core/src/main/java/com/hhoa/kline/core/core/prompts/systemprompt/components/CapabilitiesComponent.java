...
public class CapabilitiesComponent implements SystemPromptComponent {

    private final TemplateEngine templateEngine;

    @Override
    public String apply(PromptVariant variant, SystemPromptContext context) {
        String template = getCapabilitiesTemplateText(context);

        if (variant.getComponentOverrides() != null
                && variant.getComponentOverrides().containsKey(SystemPromptSection.CAPABILITIES)) {
            var override = variant.getComponentOverrides().get(SystemPromptSection.CAPABILITIES);
        ...
        String browserSupport =
                Boolean.TRUE.equals(context.getSupportsBrowserUse()) ? ", use the browser" : "";
        String browserCapabilities =
                Boolean.TRUE.equals(context.getSupportsBrowserUse())
                        ? getBrowserCapabilities()
                        : "";

        String webToolsCapabilities =
        ...
        placeholders.put("BROWSER_SUPPORT", browserSupport);
        placeholders.put("BROWSER_CAPABILITIES", browserCapabilities);
        placeholders.put("WEB_TOOLS_CAPABILITIES", webToolsCapabilities);
        placeholders.put(
                "CWD",
    ...
    public SystemPromptSection getSystemPromptSection() {
        return SystemPromptSection.CAPABILITIES;
    }

    private String getCapabilitiesTemplateText(SystemPromptContext context) {
        return """
            CAPABILITIES

            - You have access to tools that let you execute CLI commands on the user's computer, list files, view source code definitions, regex search{{BROWSER_SUPPORT}}, read and edit files%s. These tools help you effectively accomplish a wide range of tasks, such as writing code, making edits or improvements to existing files, understanding the current state of a project, performing system operations, and much more.
            ...
            """
                .formatted(
                        Boolean.FALSE.equals(context.getYoloModeToggled())
                                ? ", and ask follow-up questions"
                                : "");
    }

    private String getBrowserCapabilities() {
        return """
