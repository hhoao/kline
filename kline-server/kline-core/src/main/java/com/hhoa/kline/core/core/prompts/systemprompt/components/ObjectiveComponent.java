...
public class ObjectiveComponent implements SystemPromptComponent {
    ...
    public String apply(PromptVariant variant, SystemPromptContext context) {
        ...
        if (variant.getComponentOverrides() != null
                && variant.getComponentOverrides().containsKey(SystemPromptSection.OBJECTIVE)) {
            var override = variant.getComponentOverrides().get(SystemPromptSection.OBJECTIVE);
    ...
    public SystemPromptSection getSystemPromptSection() {
        return SystemPromptSection.OBJECTIVE;
    }
    ...
    private String getTemplateText(SystemPromptContext context) {
        ...
        return String.format(
            ...
            3. Remember, you have extensive capabilities with access to a wide range of tools that can be used in powerful and clever ways as necessary to accomplish each goal. Before calling a tool, do some analysis within <thinking></thinking> tags. First, analyze the file structure provided in environment_details to gain context and insights for proceeding effectively. Then, think about which of the provided tools is the most relevant tool to accomplish the user's task. Next, go through each of the required parameters of the relevant tool and determine if the user has directly provided or given enough information to infer a value. When deciding if the parameter can be inferred, carefully consider all the context to see if it supports a specific value. If all of the required parameters are present or can be reasonably inferred, close the thinking tag and proceed with the tool use. BUT, if one of the values for a required parameter is missing, DO NOT invoke the tool (not even with fillers for the missing params)%s. DO NOT ask for more information on optional parameters if it is not provided.
