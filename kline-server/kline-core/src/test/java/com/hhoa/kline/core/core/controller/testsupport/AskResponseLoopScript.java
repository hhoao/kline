...
/**
 * 多轮「追问 → 用户回复」脚本数据：与 {@link ScriptedConversationApiHandler#multiTurnRounds(String...)}
 * 及 {@link PendingAskResponseDriver} 配合使用。
 *
 * <p>结构：前 {@link #ASK_ROUND_COUNT} 轮 API 均输出 {@code ask_followup_question}，最后一轮输出 {@code
 * attempt_completion}；用户在每轮追问后通过 {@link com.hhoa.kline.core.core.controller.TaskManager}
 * 提交文本回复，最后对 COMPLETION_RESULT 点「是」。
 */
public final class AskResponseLoopScript {
    ...
    public static List<String> assistantApiRounds() {
        ...
        for (int i = 1; i <= ASK_ROUND_COUNT; i++) {
            rounds.add(
                    ClineToolXmlFragments.askFollowupQuestion(
                            "第" + i + "轮：请确认是否继续？"));
        }
        rounds.add(
                ClineToolXmlFragments.attemptCompletionResultOnly(
                        "六轮追问已完成，结束任务。"));
