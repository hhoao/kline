...
import com.hhoa.ai.kline.commons.utils.function.RunnableWithException;
...
public final class ExceptionUtils {
     ...
     * java.util.zip.ZipError} (a special case of InternalError).
     *
     ...
     */
    public static <T extends Throwable> T stripException(
            Throwable throwableToStrip, Class<T> typeToStrip) {
        ...
        if (typeToStrip.isAssignableFrom(throwableToStrip.getClass())) {
            return typeToStrip.cast(throwableToStrip);
        }
        return null;
    }

    public static <T extends Throwable> void stripAndRethrowException(
            Throwable throwableToStrip, Class<T> typeToStrip) throws T {
        T throwable = stripException(throwableToStrip, typeToStrip);
        if (throwable != null) {
            throw throwable;
        }
    }
     ...
     */
    public static void checkInterrupted(Throwable e) throws InterruptedException {
    ...
    public static void findAndCheckInterruptedException(Throwable e) {
    ...
    public static <T extends Exception> T logCheckAndReThrow(
            Logger log,
            Throwable e,
            Class<T> checkException,
            String msg,
            Function<String, T> exceptionFactory)
            throws T {
        @SuppressWarnings("unchecked")
        Throwable causedBy = ExceptionUtil.getCausedBy(e, checkException);
        if (causedBy != null) {
            return (T) causedBy;
        } else {
            log.error(msg, e);
            return exceptionFactory.apply(msg);
        }
    }
}
